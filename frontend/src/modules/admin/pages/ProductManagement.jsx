...
const ProductManagement = () => {
    ...
    const mainImagePreview = mainImageFile ? URL.createObjectURL(mainImageFile) : formData.mainImage;
    ...
    const handleRemoveExistingGalleryImage = (url) => {
        setFormData(prev => ({ ...prev, galleryImages: (prev.galleryImages || []).filter(u => u !== url) }));
    };

    const handleRemovePendingGalleryFile = (index) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Main image: send a replacement file if one was selected, otherwise
            // send the kept URL (or empty string to clear it).
            if (mainImageFile) {
            ...
            // Gallery images: send the kept URLs as repeated text fields (multer
            // collapses these into an array) so removed ones don't come back,
            // plus any newly selected files to add.
            const keptGalleryImages = (formData.galleryImages || []).filter(Boolean);
            if (keptGalleryImages.length > 0) {
                keptGalleryImages.forEach((url) => data.append('galleryImages', url));
            } else {
                data.append('galleryImages', '');
            }
            galleryImageFiles.forEach((file) => data.append('galleryImages', file));
                        ...
                        >
                            ...
                            <div className="flex flex-col lg:flex-row flex-1 min-h-0 min-h-[400px] max-h-[calc(100vh-200px)] overflow-hidden">
                                ...
                                <div className="flex-1 p-4 overflow-y-auto overscroll-contain touch-pan-y min-h-0">
                                    ...
                                    {modalTab === 'media' && (
                                        <div className="ds-section-spacing animate-in fade-in slide-in-from-right-2 duration-300 space-y-8">
                                            <input type="file" ref={mainImageInputRef} accept="image/*" className="hidden" onChange={handleMainImageSelect} />
                                            <input type="file" ref={galleryImageInputRef} accept="image/*" multiple className="hidden" onChange={handleGalleryImagesSelect} />

                                            <div className="space-y-3">
                                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Main Image</label>
                                                <div
                                                    role="button"
                                                    tabIndex={0}
                                                    ...
                                                    className={cn(
                                                        "relative h-48 w-48 rounded-2xl border-2 border-dashed flex items-center justify-center overflow-hidden group cursor-pointer transition-all",
                                                        mainImagePreview ? "border-slate-200 bg-slate-50/50" : "border-slate-200 hover:border-brand-500/50 hover:bg-brand-50/10"
                                                    )}
                                                >
                                                    {mainImagePreview ? (
                                                        <>
                                                            <img src={mainImagePreview} alt="Main product" className="h-full w-full object-cover" />
                                                            <button
                                                                type="button"
                                                                onClick={(e) => { e.stopPropagation(); handleRemoveMainImage(); }}
                                                                className="absolute top-2 right-2 p-1.5 rounded-full bg-white/90 text-slate-500 hover:bg-red-100 hover:text-red-600 shadow-sm"
                                                                title="Remove main image"
                                                            >
                                                                <HiOutlineXMark className="h-4 w-4" />
                                                            </button>
                                                            <div className="absolute inset-x-0 bottom-0 bg-black/50 text-white text-[10px] font-bold uppercase tracking-widest text-center py-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                Click to replace
                                                            </div>
                                                        </>
                                                    ) : (
                                                ...
                                                <p className="text-[10px] text-slate-400">The primary image shown on product cards and search results.</p>
                                            </div>
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex flex-wrap gap-3">
                                                    {(formData.galleryImages || []).map((url) => (
                                                        <div key={url} className="relative h-24 w-24 rounded-xl overflow-hidden ring-1 ring-slate-200 group">
                                                            <img src={url} alt="Gallery" className="h-full w-full object-cover" />
                                                            <button
                                                                type="button"
                                                                onClick={() => handleRemoveExistingGalleryImage(url)}
                                                                className="absolute top-1 right-1 p-1 rounded-full bg-white/90 text-slate-500 hover:bg-red-100 hover:text-red-600 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"
                                                                title="Remove image"
                                                            >
                                                    ...
                                                    {galleryImageFiles.map((file, index) => (
                                                        <div key={`pending-${index}`} className="relative h-24 w-24 rounded-xl overflow-hidden ring-1 ring-brand-200 group">
                                                            <img src={URL.createObjectURL(file)} alt="Gallery" className="h-full w-full object-cover" />
                                                            <button
                                                                type="button"
                                                    ...
                                                    <button
                                                        ...
                                                        className="h-24 w-24 rounded-xl border-2 border-dashed border-slate-200 flex items-center justify-center text-slate-400 hover:border-brand-500/50 hover:text-brand-600 hover:bg-brand-50/10 transition-all"
                                                        title="Add gallery images"
                                                    >
                                                        <HiOutlinePlus className="h-6 w-6" />
                                                    </button>
                                                </div>
                                                <p className="text-[10px] text-slate-400">Additional images shown in the product detail gallery. You can add multiple.</p>
                                            </div>
                                        </div>
                                    )}

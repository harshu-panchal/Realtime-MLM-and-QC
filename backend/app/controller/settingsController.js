...
const updateSettingsSchema = Joi.object({
  ...
  customerSplashImage: Joi.string().allow("").max(2000),
  deliverySplashImage: Joi.string().allow("").max(2000),
...
export const uploadSettingsImage = async (req, res) => {
  try {
    ...
    const validTypes = ["logo", "favicon", "categoriesbanner", "homevideobanner", "customersplash", "deliverysplash"];
    if (!validTypes.includes(type)) {
      return handleResponse(res, 400, `type must be one of: ${validTypes.join(", ")}`);
    }
